...
Puppet::Type.type(:virtual_machine).provide(:virtualbox) do
  ...
  def get_disk_uuid
    ...
    settings.each do |name, value|
      ...
      if value.downcase =~ /\.vdi|vmdk|vhd$/ then
        matches = name.match(/^\"([\w\s]+)-(\d+)-(\d+)\"$/)
        type = matches[1]
  ...
  def get_disk_details(uuid)
    ...
    details_hash = {}
    output.each do |line|
      setting = line[0..15].rstrip.chomp(":")
  ...
  def get_vm_info(name)
    # basicall if we are running it twice in a row on the same machine just return cached data
    if @info_hash then
      if @info_hash['name'] == name then
        return @info_hash
      end
    end
